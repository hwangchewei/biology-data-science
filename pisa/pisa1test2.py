...
with open('pisa1_force.txt','r') as fp:
    all_result = fp.read().split('-----------------------------------------------------------')
    all_result.pop(0)
with open('pisa1_result_lowdletaG.txt','r') as fp:
...
for result in all_result:
    ...
    if result[0] in protein_id:
        try:
            ...
            count+=1
            if (Hydrophobic-Hydrophilic) / Hydrophobic >= 0.5:
                
                count1+=1
            else:#if Hydrophilic < Hydrophobic:
                ...
                count1_list.append(result[0])
            with open('lowdeltaG_force','a') as fp:
                fp.write('%s\nHydrophobic:%f\nHydrophilic:%f\n'%(result[0],Hydrophobic,Hydrophilic))
        except:
            with open('lowdeltaG_force','a') as fp:
                fp.write('%s\nretry\n'%(result[0]))
